	...
	public static boolean importValue(int i, Row row, ListEntitas listEntitas) {
	    ...
	    if (valKosong != true) {
    		if (i == 0) {
    			...
    			if (valueDepanIsAngka == true) {
        			Pemain importPemain = new Pemain(valueList.get(1), (int) Double.parseDouble(valueList.get(2)), valueList.get(3), valueList.get(4), (int) Double.parseDouble(valueList.get(5)));
        			listEntitas.tambahPemain(importPemain);
    			}
    			else {
    			    Pemain importPemain = new Pemain(valueList.get(0), (int) Double.parseDouble(valueList.get(1)), valueList.get(2), valueList.get(3), (int) Double.parseDouble(valueList.get(4)));
        			listEntitas.tambahPemain(importPemain);
    			}
    		}
    		else if (i == 2) {
